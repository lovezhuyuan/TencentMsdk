 ...
 */JNIEXPORT void JNICALL Java_com_tencent_msdk_api_WGPlatformObserverForSO_OnRelationNotify(JNIEnv * env, jclass,
 ...
 JNIEXPORT jbyteArray JNICALL Java_com_tencent_msdk_api_WGPlatformObserverForSO_OnCrashExtDataNotify
   (JNIEnv *env, jclass){
	 ...
	 std::vector<char> extData;

	 if (WGPlatform::GetInstance()->GetObserver()) {
		 extData = WGPlatform::GetInstance()->GetObserver()->OnCrashExtDataNotify();
	 } else {
		 LOGD( "Java_com_tencent_msdk_api_WGPlatformObserverForSO_OnCrashExtDataNotify: %s","no observer! ExtData won't upload");
	 }

	 jint len = extData.size();
	 LOGD( "Java_com_tencent_msdk_api_WGPlatformObserverForSO_OnCrashExtDataNotify: extData size is %d",len);

	 jbyteArray extraByteData;

	 if(len>0){
		 extraByteData = env->NewByteArray(len);
		 //系统无法分配足够的内存
		 if(extraByteData == NULL){
			 //之前有反馈返回NULL会发生2次Crash，在此返回长度为0的数组做规避
			 extraByteData = env->NewByteArray(0);
			 return extraByteData;
		 }
		 env->SetByteArrayRegion(extraByteData, 0, len, (jbyte *)(extData.data()));
		 return extraByteData;
	 }else{
		 //之前有反馈返回NULL会发生2次Crash，在此返回长度为0的数组做规避
		 extraByteData = env->NewByteArray(0);
